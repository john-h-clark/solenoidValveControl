...
#version 4 updated to prevent threading errors caused by reading/writing to labjack simultaneously

 ##Instructions for Setting Up LabJack U12
    #You must use a U12, the python commands will not work with the U6 or other versions.
...
## Data will automatically save to the folder where the .py file is stored, unless you uncomment lines 171 and 172
...
# A mutex is a synchronization primative that ensures that two threads cannot access a
# variable at the *exact same* time. This is a common cause of sporadic and serious software
# bugs - if thread A writes to location that thread B reads from at the *exact same* CPU cycle,
# then all kinds of catastrophic failures can occur
# Acquiring a mutex tells the CPU to essentially stop multithreading - only one thread can run
# while the mutex is acquired. Because of this, it is crucial that the processing time of whatever
# happens between "acquire" and "release" is as short as possible - no other processing can occur
# on the CPU while the mutex is locked.
mutex = Lock()
...
def btn1_click():
    ...
    current_time = datetime.datetime.now().strftime("%H:%M:%S") #convert time to string
    ...
    global AO0_volts #declare global variable so value carries over with button callback
    global AO1_volts #declare global variable so value carries over with button callback
    #Change AO0 Voltage
    AO0_volts = 3.33
    mutex.acquire()
    d.eAnalogOut(AO0_volts, AO1_volts) #set output voltage on LabJack - open filter valve
    mutex.release()
    ...
    mutex.acquire()
    d.eAnalogOut(AO0_volts, AO1_volts) #set output voltage on LabJack - close sample valve
    mutex.release()
    #Sleep
...
def btn2_click():
    ...
    AO1_volts = 3.33
    mutex.acquire()
    d.eAnalogOut(AO0_volts, AO1_volts) #set output voltage on LabJack - open sample valve
    mutex.release()
    sleep(1) # waiting period when both valves are open
    ...
    AO0_volts = 0.0
    mutex.acquire()
    d.eAnalogOut(AO0_volts, AO1_volts) #set output voltage on LabJack - close filter valve
    mutex.release()
    sleep(.1) #buffer to stop buttons from being pressed too quickly
...
box1 = canvas.create_rectangle(25, 25, 85, 50, fill='gold') # initialize with filter valve open
...
##path = ("C:/Users/AMS/Documents/Filter Valve Control") #define path to save data
##os.chdir(path) #change working directory to path
print("Current working directory: {0}".format(os.getcwd())) #print current directory
...
while file_exists: #loop until filename does not exist in directory
    file_name = ("valve_data_" + file_date + "_" + str(n) + ".txt") #create new filename with _n
...
##file1.write("Date, Time, Valve 1 Output, Valve 2 Output, Valve 1 Position, Valve 2 Position\n") #write column headers
file1.write("DateTime, Valve 1 Output, Valve 2 Output, Valve 1 Position, Valve 2 Position, Vaisala_CH1V, Vaisala_CH2V, Temp, RH n")
...
def write_data_bkgd():
    while True:
        try:
            ...
            #current_time = time_now.strftime("%H:%M:%S") #convert time to string
            #current_date = time_now.strftime("%m/%d/%Y") #convert date to string
            ...
            #get valve 2 position (pin 18 = open = 1, pin 16 = closed = 0, no data = -999)
            ...
            #rowData = [current_date, current_time, str(AO0), str(AO1), str(valve1), str(valve2), "\n"] #create a row of data
            rowData = [current_datetime, str(AO0), str(AO1), str(valve1), str(valve2), str(AO2), str(AO3), str(Temp), str(RH) "\n"] #create a row of data
